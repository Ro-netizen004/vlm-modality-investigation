#!/usr/bin/env python3
"""CLI entry point for VLM math benchmark runs (any registered dataset)."""
...
from vlm_benchmark.config import DEFAULT_CONFIG, finalize_config
from vlm_benchmark.datasets import load_dataset
from vlm_benchmark.experiments.runner import run_experiment, summarize_results
from vlm_benchmark.models import load_model
from vlm_benchmark.visualize import plot_results


def parse_args():
    p = argparse.ArgumentParser(
        description="Run VLM modality benchmark (dataset-agnostic via --dataset-type)"
    )
    ...
    p.add_argument(
        "--mode",
        choices=["text_only", "image_only", "text_and_image", "mismatch"],
        default=DEFAULT_CONFIG["experiment_mode"],
    )
