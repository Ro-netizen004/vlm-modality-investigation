#!/bin/bash -l
# ── Run 6 remaining models in parallel (12h time limit) ──
# After downgrading: pip install transformers==4.45.2
#
# Usage:
#   cd ~/vlm-modality-research
#   bash scripts/gaivi_run_final6.sh

PARTITION="CISL"
REPO_DIR="$HOME/vlm-modality-research"
OUTPUT_DIR="$HOME/vlm_research_results"
mkdir -p "$OUTPUT_DIR" "$REPO_DIR/logs"

MODELS=(
    "llava-hf/llava-v1.6-mistral-7b-hf"
    "Qwen/Qwen2.5-VL-7B-Instruct"
    "llava-hf/llava-onevision-qwen2-7b-ov-hf"
    "microsoft/Phi-3.5-vision-instruct"
    "openbmb/MiniCPM-V-2_6"
    "OpenGVLab/InternVL2-8B"
)

echo "Submitting ${#MODELS[@]} jobs (12h limit each)..."
echo ""

for MODEL in "${MODELS[@]}"; do
    SHORT=$(echo "$MODEL" | rev | cut -d'/' -f1 | rev)

    sbatch <<SCRIPT
#!/bin/bash -l
#SBATCH --job-name=vlm-${SHORT:0:15}
#SBATCH -p ${PARTITION}
#SBATCH -w GPU53
#SBATCH --gpus=1
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=12:00:00
#SBATCH --mail-user=aviralgupta@usf.edu
#SBATCH --mail-type=END,FAIL
#SBATCH --output=logs/${SHORT}_final_%j.log
#SBATCH --error=logs/${SHORT}_final_%j.err

conda activate vlm

echo "============================================"
echo "  Model: ${MODEL}"
echo "  Node:  \$(hostname)"
echo "  GPU:   \$(nvidia-smi --query-gpu=name,memory.total --format=csv,noheader | head -1)"
echo "  Date:  \$(date)"
echo "============================================"

cd ${REPO_DIR}

srun python scripts/run_benchmark.py \\
    --config configs/gaivi.yaml \\
    --models "${MODEL}" \\
    --output-dir "${OUTPUT_DIR}" \\
    --hf-images

echo "Done: ${MODEL} at \$(date)"
SCRIPT

    echo "  Submitted: $SHORT"
done

echo ""
echo "All ${#MODELS[@]} jobs submitted (12h limit each)!"
echo "Monitor with: squeue -u \$USER"
echo "Results will appear in: $OUTPUT_DIR"
